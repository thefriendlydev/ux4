<?php
function cjpopups_backend_report_breadcrumbs(){
	$sep = ' &nbsp;/&nbsp; ';
	$report_url = cjpopups_string(cjpopups_callback_url());
	...
	if(isset($_GET['report']) && $_GET['report'] == 'alltime'){
		$output[] = __('All Time', 'cjpopups');
	}
	if(isset($_GET['report']) && $_GET['report'] == 'yearly'){
		$output[] = '<a href="'.$report_url.'report=alltime">'.__('All Time', 'cjpopups').'</a>';
		$output[] = $sep;
		$output[] = $_GET['ry'];
